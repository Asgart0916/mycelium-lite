> ⚠️ **已封存（v1，2026-05-31）** — 這是舊定位「連結浮現」的設計文件，對應 M0–M4a 已實作 code（commit `a0dd2f3`）。
> 2026-05-30 起專案轉型為「單人 design sprint 引導器」（純前端 TS），現行規格見 `Design.md`。
> 本檔保留僅為決策可追溯，**不再施工**。轉型理由與失敗風險見 `PreMortem.md`。

---

# mycelium-lite — 設計文件（v1，已封存）
